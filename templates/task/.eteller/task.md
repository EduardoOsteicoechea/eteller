# Task — <task-id>

**Wave:** wave-N  
**Branch:** <branch>  
**Clone cwd:** `waves/wave-N/<task-id>/<REPO_DIR_NAME>/`  
**Coding law (absolute path — read-only):** `<workspace>/base/<REPO_DIR_NAME>/CurrentTask/*AGENT_SPEC*.md`  
**Do not** treat this clone’s `CurrentTask/*AGENT_SPEC*` as law.

**Reports:** `Reports/<task-id>.txt` (update on completion)  
**Milestones:** see sibling `progress.md`

## Completion

1. Update `Reports/<task-id>.txt`
2. Open PR → `INTEGRATION_BRANCH` (do **not** merge unless user asks)
3. Set `state.md` to closed with PR URL
4. Commit + push on this wave-task branch
