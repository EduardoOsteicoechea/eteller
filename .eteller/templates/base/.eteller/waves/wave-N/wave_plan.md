...
<from user story — shared-functionality coupling that defines this wave>

## Tasks

| id | branch | slot path | status | in-wave deps | PR |
|----|--------|-----------|--------|--------------|-----|
| *(none yet — awaiting user story)* | | | | | |

Do not invent task ids or branches. Copy exact names from the user’s steer / orchestration.md.

Do not start coding this wave until the user explicitly asks to start it (see workflow hard gate B).
