...
5. Update `<clone>/.eteller/state.md` and `progress.md` (on approve: `status: awaiting_ux_review`; next gate is UXReview)
6. Commit + push on that wave-task branch (`approved.md`, `progress.md`, `state.md`)

**Reviewer gates (product AGENT_SPEC):** block on missing triple Release build evidence when code changed. Do **not** block on per-task Revit IT or missing QA HTML on the task branch. Do **not** write `ux_review.md` (separate UXReview step).
